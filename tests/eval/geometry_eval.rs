...
fn gate_10_the_lens_correction_moves_a_protected_region_before_the_filter_sees_it() {
    ...
    let plan = planner.plan(&input);
    assert!(
        plan.lens.corrects_distortion(),
        "the 14 mm was not corrected"
    );
    let scale = lens::valid_scale(plan.lens.distortion, input.aspect);
    let mapped = lens::map_rect(
        input.regions[0].rect,
        plan.lens.distortion,
        input.aspect,
        scale,
    );
    for variant in &plan.crops {
    ...
    for k in [
        ...
        for aspect in [1.5_f32, 0.6667] {
            ...
            for point in [[0.1_f32, 0.2], [0.5, 0.5], [0.92, 0.88]] {
                ...
                let rendered = aura_raw::colour::lens::source_of(point, k, aspect, render_scale);
