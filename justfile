...
# Verify this machine's AI provider from .claude/settings.json and store it
# through the product's own commands: a text probe, a vision probe, the key in
# the credential store, the choice in the catalog, and the Settings panel's own
# Check path read back. Refuses without writing if the endpoint does not answer
# a vision call, because PhotoAutoEdit sends images and a text-only model would
# fall back locally forever without a word.
ai-provision:
    cargo run --package xtask -- ai-provision
