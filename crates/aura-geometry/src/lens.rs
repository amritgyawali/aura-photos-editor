...
pub fn decide(
    ...
    reasons.push(GeometryReason::plain(GeometryCode::LensProfileMissing, -0.05));
    (
        LensCorrection {
            lens_id: input.lens_id.clone(),
...
mod tests {
    ...
    fn a_face_box_moves_when_the_optics_are_corrected() {
        ...
        let centre_shift =
            ((still.x - centred.x).powi(2) + (still.y - centred.y).powi(2)).sqrt();
        assert!(
    ...
    fn a_measured_profile_corrects_fringing_and_an_estimated_one_does_not() {
        ...
        assert!(!estimated.corrects_ca(), "an estimate must not touch fringing");
    ...
    fn the_vignette_correction_is_partial_by_design() {
        ...
        assert!(correction.vignette < 0.5, "full correction flattens the frame");
    }
}
