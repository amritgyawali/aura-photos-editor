...
impl GeometryStore {
    ...
    ///
    /// `AURA-DB-3006` when the query fails.
    pub fn variant(&self, image: ImageId, purpose: CropPurpose) -> AuraResult<Option<CropVariant>> {
        let key = image.to_db();
    ...
    pub fn outline(
        ...
        self.catalog.read(move |conn| {
            ...
            let mut cursor = missing
                .query(params![
                    key,
                    i64::try_from(GeometryOutline::MAX_MISSING).unwrap_or(20)
                ])
                .map_err(|e| statement_failed("could not read the missing profiles", &e))?;
            while let Some(row) = cursor
                .next()
                .map_err(|e| statement_failed("could not read the missing profiles", &e))?
            {
                outline
                    .missing_profiles
                    .push(row.get(0).unwrap_or_default());
            }
            Ok(outline)
        })
    }
    ...
    pub fn accept(&self, image: ImageId) -> AuraResult<()> {
        ...
        if changed == 0 {
            return Err(errors::framing_refused(
                "the photograph has no geometry plan",
            ));
        }
        Ok(())
    }
    ...
    pub fn check_versions(&self, project: &ProjectId, current: (u16, u16, u16)) -> AuraResult<()> {
        let key = project.to_db();
...
fn decode_reasons(text: &str) -> Vec<GeometryReason> {
    ...
    items
        ...
        .filter_map(|item| {
            ...
            let weight = object.get("w").and_then(Value::as_f64).unwrap_or(0.0) as f32;
            let text = object
                .get("t")
...
fn original_of(plan: &GeometryPlan, frame_aspect: f32) -> CropVariant {
    ...
    if plan.rotate_deg.abs() >= f32::EPSILON {
        let aspect = if frame_aspect > 0.0 {
            frame_aspect
        } else {
            1.5
        };
...
///
/// `AURA-DB-3006` when the query fails.
pub fn scene_histogram(
    catalog: &Catalog,
    project: &ProjectId,
) -> AuraResult<BTreeMap<String, u32>> {
    let key = project.to_db();
