    ...
    pub fn parse(text: &str) -> AuraResult<Self> {
        ...
        for scene in &raw.scene {
            let id = SceneId::ALL
                ...
                .ok_or_else(|| {
                    errors::rules_refused(format!("unknown scene '{}'", scene.id))
                })?;
        ...
        Ok(Self {
            rows,
            defaults,
            version: if raw.version > 0 { raw.version } else { RULES_VER },
        })
    }
...
fn convert(raw: &RawRow, id: &str) -> AuraResult<SceneRule> {
    ...
    for name in raw.variants.clone().unwrap_or_default() {
        let purpose = CropPurpose::ALL
            ...
            .ok_or_else(|| errors::rules_refused(format!("row '{id}': unknown variant '{name}'")))?;
