...
// geometry
//
// RETIRED. `stage_geometry` lived here until phase 23, which moved it into `geometry.wgsl`
// beside the perspective warp it now composes with and the two lens resamples that share its
// bilinear read. Phase 20 made the same move when `retouch_apply.wgsl` took `stage_retouch` out
// of this file, and wrote down the reason: two entry points with one name is the drift
// `shader_parity.rs` exists to catch, and the cheapest moment to avoid it is when the second one
// is written.
// ---------------------------------------------------------------------------
