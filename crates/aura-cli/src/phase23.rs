...
pub fn verify(args: &[String]) -> ExitCode {
    ...
    match forbidden_columns(&catalog) {
        ...
        Ok(found) => {
            eprintln!("  migration 20 grew a forbidden column: {}", found.join(", "));
            failures += 1;
        }
        Err(err) => {
    ...
    for case in &cases {
        ...
        if case.must_keep_framing && !plan.kept_original_framing() {
            eprintln!("  {}: the framing was changed and must not have been", case.name);
            failures += 1;
        }
        ...
        match service.of_image(image) {
            Ok(Some(after)) if after.user_edited => {
                ...
                if (primary.rect.x - chosen.x).abs() < 1e-3 {
                    println!("  and the rectangle the photographer chose is still the one delivered");
                } else {
        ...
        for table in ["geometry_plan", "geometry_crop"] {
                ...
                .map_err(|e| aura_core::errors::db::statement_failed("table_info", &e))?
            {
                ...
                let tokens: Vec<String> = name
                    ...
                    .collect();
                for needle in [
                    "pixel", "pixels", "blob", "thumb", "thumbnail", "render", "path", "file",
                    "fill", "inpaint", "applied", "deleted", "matte", "alpha",
                ] {
